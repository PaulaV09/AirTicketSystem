...
public class GetLicensesExpiringSoonUseCase
{
    ...
    public async Task<IEnumerable<PilotLicenseEntity>> ExecuteAsync(int diasUmbral)
    {
        if (diasUmbral <= 0)
            throw new ArgumentException(
                "El umbral de días debe ser mayor a 0.");

        if (diasUmbral > 365)
            throw new ArgumentException(
                "El umbral no puede superar 365 días.");

        return await _repository.GetProximasAVencerAsync(diasUmbral);
    }
}